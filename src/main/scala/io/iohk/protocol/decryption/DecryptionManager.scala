...
*/
class DecryptionManager(ctx:              CryptoContext,
                        ballots:          Seq[Ballot]) {
  ...
  lazy val delegationsSum: Seq[ElGamalCiphertext] = Tally.computeDelegationsSum(ctx, votersBallots)
  ...
  def computeDelegations(delegationsC1: Seq[Seq[GroupElement]]): Seq[BigInt] = {
    Tally.decryptVectorOnC1(ctx, delegationsC1, delegationsSum)
  }

  def computeChoicesSum(delegations: Seq[BigInt]): Seq[ElGamalCiphertext] = {
    Tally.computeChoicesSum(ctx, votersBallots, expertsBallots, delegations)
  }

  def computeTally(choicesC1: Seq[Seq[GroupElement]], delegations: Seq[BigInt]): Try[Tally.Result] = {
    Tally.countVotes(ctx, votersBallots ++ expertsBallots, choicesC1, delegations)
  }
}
