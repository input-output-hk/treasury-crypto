...
//  val defaultBase: BigInt = BigInt(Math.pow(2, 8).toInt) // segment size is 8 bits (all segments are modulo 2^8)
  val defaultBase: BigInt = BigInt(Math.pow(2, 32).toLong) // segments size is 32 bits (all segments are modulo 2^32)
