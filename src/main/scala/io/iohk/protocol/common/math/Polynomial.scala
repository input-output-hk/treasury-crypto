...
/**
  ...
  * @param coeffs_in all other coefficients a_1,..,a_t; Can be empty, then coefficients are generated with drng
  */
case class Polynomial(dlogGroup: DiscreteLogGroup, degree: Int, a_0: BigInt, coeffs_in: Seq[BigInt] = Seq()) {
  ...
  private val polynomial = {
    Seq(a_0) ++ {
      if(coeffs_in.isEmpty){
        ...
        require(coeffs_in.length == degree, "Number of coefficients is inconsistent with the specified degree")
        coeffs_in
      }
    }
  }
  ...
  def evaluate(x: BigInt): BigInt = {
    polynomial.zipWithIndex.foldLeft(BigInt(0)){
      (sum, coeff_index) =>
        val (c, i) = coeff_index
        (sum + c * x.modPow(BigInt(i), modulus)) mod modulus
    }
  }
