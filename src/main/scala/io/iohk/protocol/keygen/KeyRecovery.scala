...
class KeyRecovery(ctx: CryptoContext, cmIdentifier: Identifier[Int]) {
  ...
  def restorePrivateKeys(disqualifiedCommitteeIds: Set[Int],
    ...
    val restoredKeys = disqualifiedCommitteeIds.map{ id =>
      ...
      val recoveryShares = violatorsSharesData.map(_.violatorsShares.find(_.issuerID == id).map(_.share_a)).flatten
  ...
  def prepareRecoverySharesData(committeeMemberKey: KeyPair,
    ...
    if (disqualifiedCommitteeIds.nonEmpty) {
      ...
      val recoveryShares = disqualifiedCommitteeIds.toSeq.map { id =>
        DistrKeyGen.generateRecoveryKeyShare(ctx, cmIdentifier,
  ...
  def verifyRecoverySharesData(committePubKey: PubKey,
    ...
    require (violatorsSharesData.violatorsShares.map (_.issuerID).toSet == disqualifiedCommitteeIds, "Unexpected set of recovery shares")

    violatorsSharesData.violatorsShares.foreach { share =>
      require(DistrKeyGen.validateRecoveryKeyShare(ctx, cmIdentifier, committePubKey, dkgR1DataAll, share).isSuccess)
    }
  }
}
