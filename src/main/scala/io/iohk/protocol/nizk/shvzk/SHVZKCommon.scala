...
class SHVZKCommon(pubKey: PubKey, unitVector: Seq[ElGamalCiphertext])
  ...
  /* Use hash of the public key as the common reference string for both prover and verifier. It should be fine
  * since pk is generated by distributed key generation protocol and fixed for all provers, so they can not influence its distribution */
  protected val crs = BigInt(hashFunction.hash(pubKey.bytes))
