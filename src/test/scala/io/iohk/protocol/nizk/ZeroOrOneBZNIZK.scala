...
import io.iohk.protocol.nizk.unitvectornizk.ZeroOrOneBZNIZK
...
class ZeroOrOneBZNIZKTest extends FunSuite {
  ...
  test("encrypt one") {
    ...
    val proof = ZeroOrOneBZNIZK.produceNIZK(pubKey, 1, c, r).get
    val verified = ZeroOrOneBZNIZK.verifyNIZK(pubKey, c, proof)
  ...
  test("encrypt zero") {
    ...
    val proof = ZeroOrOneBZNIZK.produceNIZK(pubKey, 0, c, r).get
    val verified = ZeroOrOneBZNIZK.verifyNIZK(pubKey, c, proof)
  ...
  test("encrypt two") {
    ...
    val proof = ZeroOrOneBZNIZK.produceNIZK(pubKey, m, c, r).get
    val verified = ZeroOrOneBZNIZK.verifyNIZK(pubKey, c, proof)
