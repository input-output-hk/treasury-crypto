...
class TallyRound1Test extends FunSuite with TallyTestSetup {
  ...
  test("executeRound1 should update tally phase properly") {
    ...
    val tally3 = new Tally(new ProtocolContext(ctx, 3, 0), cmIdentifier, Map())
    ...
    val tally4 = new Tally(new ProtocolContext(ctx, 3, 0), cmIdentifier, committeeKeys.map(x => (x._2 -> Some(x._1))).toMap)
