...
import io.iohk.protocol.resharing.ResharingTests.{corruptResharings, getSharesPerParty, initialize, reshareSharesOfTheParty}
...
object ResharingTests {

  def generateKeys(num: Int)(implicit context: CryptoContext): Seq[KeyPair] = for (_ <- 0 until num) yield encryption.createKeyPair(context.group).get
  ...
  def initialize(partiesNum: Int)
                (implicit context: CryptoContext): (Seq[Resharing], Int, Seq[KeyPair], SharingParameters) = {
  ...
  def getSharesPerParty(secrets: Seq[BigInt], params: SharingParameters)
                       (implicit context: CryptoContext): Seq[Seq[Share]] = {
  ...
  def reshareSharesOfTheParty(partyShares: Seq[Share], params: SharingParameters)
                             (implicit context: CryptoContext): ResharingData = {
