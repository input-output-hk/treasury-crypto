...
import io.iohk.protocol.voting.ballots.{BallotSerializer, PrivateVoterBallot}
import org.scalatest.FunSuite

class PrivateVoterBallotTest extends FunSuite {
  val ctx = new CryptoContext(None)
  import ctx.{group}

  val (privKey, pubKey) = encryption.createKeyPair.get

  test("creation of PrivateVoterBallot") {
    val pctx = new ProtocolContext(ctx, 3, 5)
    val stake = 13

    // test all possible votes
    for (vote <- 0 until (pctx.numberOfExperts + pctx.numberOfChoices)) {
      val ballot = PrivateVoterBallot.createBallot(pctx, 0, vote, pubKey, stake).get
    ...
    for (vote <- invalidVotes) {
      val badBallot = PrivateVoterBallot.createBallot(pctx, 0, vote, pubKey, stake)
      require(badBallot.isFailure)
    }
  }

  test("PrivateVoterBallot serialization") {
    val pctx = new ProtocolContext(ctx, 3, 5)
    val stake = 13
    val vote = 2
    ...
    val recoveredBallot = BallotSerializer.parseBytes(bytes, Option(group)).get.asInstanceOf[PrivateVoterBallot]
    ...
    val recoveredBallot2 = BallotSerializer.parseBytes(ballotWithoutProofs.bytes, Option(group)).get.asInstanceOf[PrivateVoterBallot]
