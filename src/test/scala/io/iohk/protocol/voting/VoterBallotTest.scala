...
import io.iohk.protocol.voting.ballots.{BallotSerializer, VoterBallot}
...
class VoterBallotTest extends FunSuite {
  ...
  test("creation of VoterBallot") {
    ...
    for (vote <- invalidVotes) {
      val badBallot = VoterBallot.createBallot(pctx, 0, vote, pubKey, stake)
      require(badBallot.isFailure)
    }
  }

  test("VoterBallot serialization") {
    val pctx = new ProtocolContext(ctx, 3, 5)
    val stake = 13
    val vote = 2
    ...
    val recoveredBallot = BallotSerializer.parseBytes(bytes, Option(group)).get.asInstanceOf[VoterBallot]
    ...
    val recoveredBallot2 = BallotSerializer.parseBytes(ballotWithoutProofs.bytes, Option(group)).get.asInstanceOf[VoterBallot]
  ...
  test("VoterBallot serialization when there are no experts") {
    val pctx = new ProtocolContext(ctx, 3, 0)

    val ballot = VoterBallot.createBallot(pctx, 0, 1, pubKey, 1).get
    val bytes = ballot.bytes
    val recoveredBallot = BallotSerializer.parseBytes(bytes, Option(ctx.group)).get.asInstanceOf[VoterBallot]
