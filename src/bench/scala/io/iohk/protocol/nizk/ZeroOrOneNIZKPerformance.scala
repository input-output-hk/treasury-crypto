...
class ZeroOrOneNIZKPerformance {
  ...
  def run() = {
    ...
    TimeUtils.accurate_time("BZ NIZK creation (for single ciphertext): ", ZeroOrOneBZNIZK.produceNIZK(pubKey, 1, c, r).get)
    val proofBZ = ZeroOrOneBZNIZK.produceNIZK(pubKey, 1, c, r).get
    ...
    TimeUtils.accurate_time("SigmaOR NIZK creation (for single ciphertext): ", ZeroOrOneSigmaNIZK.produceNIZK(pubKey, 1, c, r).get)
    val proofSigma = ZeroOrOneSigmaNIZK.produceNIZK(pubKey, 1, c, r).get
